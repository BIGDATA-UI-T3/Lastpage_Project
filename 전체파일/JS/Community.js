...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    const avatarImg = profileCard.querySelector(".avatar");
    const nameField = profileCard.querySelector("h3");
    const userIdField = profileCard.querySelector(".user-id");
    const bioField = profileCard.querySelector(".profile-bio");
    const myPostsBtn = document.getElementById('my-posts-view-btn');
    ...
    if (!data.isLogin) {
        // --- 로그인 안 된 상태 UI ---
        avatarElement.textContent = "account_circle"; // 기본 아이콘 유지
        nameField.textContent = "로그인이 필요합니다";
        ...
        loginBtn.classList.add("my-posts-btn");
        loginBtn.onclick = () => location.href = "/signin"; // 로그인 페이지 경로
        profileCard.appendChild(loginBtn);
    ...
    bioField.textContent = data.bio ? data.bio : "나를 표현하는 소개글을 작성해보세요.";
    ...
    if (data.avatar && data.avatar.trim() !== "") {
        ...
        img.src = data.avatar;
        img.classList.add("avatar");  // 기존 스타일 유지
        ...
        // --- 프로필 이미지 없으면 아이콘 유지 ---
        avatarElement.textContent = "account_circle";
    }
});
