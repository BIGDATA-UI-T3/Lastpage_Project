...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (function setDateLimits() {
    ...
    qs('#targetDate').min = `${y2}-${m2}-${d2}`;
    qs('#kitDate').min = todayStr;
    qs('#visitDate').min = todayStr;
  })();
  ...
  petTypeEl.addEventListener('change', () => {
    ...
    if (type === 'other') {
      ...
      petBreedText.style.display = 'none';
      petBreedSel.innerHTML = '<option value="">품종 선택</option>' +
  ...
  function validateStep1() {
    ...
    Object.assign(state, { ownerName: name, ownerPhone: phone, ownerEmail: email, ownerAddr: addr, petName: pnm, petWeight: pwt });

    const emailOK = /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email);
    const breedOK = (state.petType === 'other') ? !!state.petBreed : (!!state.petType && !!state.petBreed);
  ...
  qs('#metal').addEventListener('change', e => { state.metalColor = e.target.value; validateStep2(); });
  qs('#chainLength').addEventListener('change', e => { state.chainLength = e.target.value; validateStep2(); });
  qs('#ringSize').addEventListener('input', e => { state.ringSize = e.target.value.trim(); validateStep2(); });
  qs('#qty').addEventListener('input', e => { state.quantity = Math.max(1, parseInt(e.target.value || '1', 10)); validateStep2(); });
  qs('#engrave').addEventListener('input', e => { state.engravingText = e.target.value; validateStep2(); });
  qs('#font').addEventListener('change', e => { state.engravingFont = e.target.value; validateStep2(); });
  ...
  qsa('.seg-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      qs('.seg-btn.is-active')?.classList.remove('is-active');
  ...
  qs('#targetDate').addEventListener('change', e => { state.targetDate = e.target.value; validateStep3(); });
  qs('#express').addEventListener('change', e => { state.isExpress = e.target.checked; validateStep3(); });
  qs('#kitAddr').addEventListener('input', e => { state.kitAddr = e.target.value.trim(); validateStep3(); });
  qs('#kitDate').addEventListener('change', e => { state.kitDate = e.target.value; validateStep3(); });
  qs('#visitDate').addEventListener('change', e => { state.visitDate = e.target.value; validateStep3(); });
  qs('#visitTime').addEventListener('change', e => { state.visitTime = e.target.value; validateStep3(); });
  qs('#tracking').addEventListener('input', e => { state.trackingInfo = e.target.value.trim(); });
  ...
  function validateStep3() {
    ...
    if (state.shipMethod === 'kit') ok = ok && !!state.kitAddr && !!state.kitDate && !!state.kitTime;
    if (state.shipMethod === 'visit') ok = ok && !!state.visitDate && !!state.visitTime;
  ...
  qs('#confirmChk').addEventListener('change', () => qs('#submitBtn').disabled = !qs('#confirmChk').checked);
  ...
  qs('#submitBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      if (res.ok) {
        alert(state.id ? '예약이 성공적으로 수정되었습니다!' : '예약이 성공적으로 저장되었습니다!');
